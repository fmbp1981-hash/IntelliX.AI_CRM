...
import { useCreateDealTemplate } from '../../hooks/useDealTemplates';
...
export const DealDetailModal: React.FC<DealDetailModalProps> = ({ dealId, isOpen, onClose }) => {
  ...
  const { mutateAsync: createTemplate } = useCreateDealTemplate();
  ...
  const handleSaveTemplate = async () => {
    if (!deal || !templateName.trim() || !profile?.organization_id) return;
    setIsSavingTemplate(true);
    try {
      await createTemplate({
        name: templateName,
        board_id: deal.boardId,
        defaults: {
          title_prefix: deal.title,
          value: deal.value,
  ...
  const inner = (
    <>
                  ...
                  <div className="bg-linear-to-br from-primary-50 to-white dark:from-primary-900/10 dark:to-dark-card p-6 rounded-xl border border-primary-100 dark:border-primary-500/20">
                    ...
                    {dealBoard?.agentPersona && (
                      <div className="mb-6 bg-slate-900/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg p-3 flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-linear-to-br from-teal-500 to-indigo-600 flex items-center justify-center text-white shadow-lg">
                          <Bot size={20} />
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-teal-600 dark:text-teal-400 bg-teal-100 dark:bg-teal-900/30 px-1.5 py-0.5 rounded">
                              Atuando como
                            </span>
                          </div>
      ...
      <ConfirmModal
        ...
        title="Salvar como Template"
        message="Este negócio será salvo como um template com seus campos, valores e tags atuais."
        confirmText={isSavingTemplate ? "Salvando..." : "Salvar Template"}
      >
        <div className="mt-4">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-primary-500 text-slate-900 dark:text-white"
          />
        </div>
      </ConfirmModal>
    </>
